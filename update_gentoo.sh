...
if [[ "$response" =~ ^(yes|y)$ ]]
then
  emerge -1 neovim tig llvm-common tmux
fi
...
then
  ...
  emerge -1 --keep-going clang llvm compiler-rt compiler-rt-sanitizers llvm-libunwind clang-runtime libcxxabi libcxx libomp mesa lld lldb
